...
from local_tests.test_helpers.generators import Generators
...
class JsonHelper():
    ...
    @staticmethod
    def create_json_file_filled(records_count = 1, last_record_not_finished = False) -> str:
        ...
        for num in range(records_count):
            ...
            with open(file = filename, mode = 'r+', encoding="utf-8") as file:
                ...
                if num == records_count - 1 and last_record_not_finished:
                    new_record.append({
                        const.JSON_TASK: Generators.generate_string(Generators.generate_number(1, 100), 'random'),
                        const.JSON_CATEGORY: Generators.generate_string(Generators.generate_number(1, 20), 'random'),
